...
import com.tencent.opensource.model.EndLessOnScrollListener;
import com.tencent.opensource.model.HttpUtils;
import com.tencent.opensource.model.Music;
import com.tencent.opensource.model.prmusic;
...
public class AudioEffectPanel extends BaseBottomSheetDialog {
    ...
    private void initView() {
        ...
        mRVAudioBGM.setLayoutManager(manager);
        mRVAudioBGM.setAdapter(mBGMRVAdapter);
        mRVAudioBGM.addOnScrollListener(new EndLessOnScrollListener(manager) {
            @Override
        ...
        mSwitchMusiceAudiction.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (mTRTCVoiceRoom != null) {
                    mTRTCVoiceRoom.setVoiceEarMonitorEnable(isChecked);
                }
    ...
    private void loadMoreData() {
        ...
        HttpUtils.RequestPost(BuildConfig.HTTP_API + "/music?page=" + totpage, params, new HttpUtils.HttpListener() {
            @Override
            public void success(String response) {
                prmusic prmusic = new Gson().fromJson(response, prmusic.class);
                for (Music music : prmusic.getMusic()) {
                    mBGMItemEntityList.add(new BGMItemEntity(music.getTitle(), music.getUrl()));
                }
                if (prmusic.getMusic().size() > 0) {

                    if (mTvBGM!=null) {
                        mTvBGM.post(new Runnable() {
                            @Override
                            public void run() {
                                mBGMRVAdapter.notifyDataSetChanged();
                            }
                        });
                    }
                } else {
    ...
    /**
     * 音乐列表   音乐来源于https://www.9ku.com/
     *
     * @return
     */
    private List<BGMItemEntity> createBGMItems() {
        mBGMItemEntityList = new ArrayList<>();
        Map<String, String> params = new HashMap<>();
        params.put(Constants.USERID, userInfo.getUserId());
        params.put(Constants.TOKEN, userInfo.getToken());
        totpage++;
    ...
    public class BGMRecyclerViewAdapter extends RecyclerView.Adapter<BGMRecyclerViewAdapter.ViewHolder> {
        ...
        public class ViewHolder extends RecyclerView.ViewHolder {
            ...
            public void bind(final BGMItemEntity model, final int positon, final OnItemClickListener listener) {
                mTitleTv.setText(model.mTitle);
                itemView.setOnClickListener(v -> handleBGM(positon, model));
            }
