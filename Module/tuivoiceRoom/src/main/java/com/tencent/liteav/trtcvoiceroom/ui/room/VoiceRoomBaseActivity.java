...
public class VoiceRoomBaseActivity extends AppCompatActivity implements VoiceRoomSeatAdapter.OnItemClickListener, TRTCVoiceRoomDelegate, InputTextMsgDialog.OnTextSendListener, MsgListAdapter.OnItemClickListener {
    ...
    protected int mRoomId;                  //房号ID
    protected String mRoomName;             //房号名称
    protected String mUserName;             //名称
    protected String mUserAvatar;           //头像
    protected String mRoomCover;            //封号
    protected String mMainSeatUserId;       //主播
    ...
    private GiftInfoDataHandler mGiftInfoDataHandler;   //查询礼物
    private GiftAnimatorLayout mGiftAnimatorLayout;     //插入礼物
    ...
    protected void initListener() {
        ...
        //跳转到个人主页
        mImgHead.setOnClickListener(onClickListener);
        mIvAnchorHead.setOnClickListener(onClickListener);
    ...
    protected void initData() {
        ...
        mTRTCVoiceRoom.setDelegate(this); //接收端：监听自定义消息
    ...
    private void showGiftPanel() {
        ...
        giftPanelView.setGiftPanelUser(mMainSeatUserId, mSelfUserId); // mAnchorInfo//主播对像 //用户mSelfUserId
        giftPanelView.setGiftPanelDelegate(giftPanelDelegate);
        giftPanelView.show();
    }
    ...
    private void sendGift(GiftInfo giftInfo) {
        ...
        giftInfoCopy.sendUser = mContext.getString(R.string.trtcvoiceroom_me);
        giftInfoCopy.sendUserHeadIcon = userModel.userAvatar;
        mGiftAnimatorLayout.show(giftInfoCopy);

        //创建一条JSON消息
        GiftSendJson jsonData = new GiftSendJson();
        jsonData.setSendUser(userModel.userName);
        jsonData.setSendUserHeadIcon(userModel.userAvatar);
        ...
        // 发送端：您可以通过自定义 Cmd 来区分弹幕和点赞消息
        // eg:"CMD_DANMU"表示弹幕消息，"CMD_LIKE"表示点赞消息
        //mTRTCVoiceRoom.sendRoomCustomMsg("CMD_DANMU", "你好啊，小姐姐", null);
        //mTRTCVoiceRoom.sendRoomCustomMsg("CMD_LIKE", "", null);

        Log.d(TAG, "sendGift: " + new Gson().toJson(jsonData));

        //自定义消息常用于传输自定义信令，例如用于点赞消息的发送和广播
        mTRTCVoiceRoom.sendRoomCustomMsg(String.valueOf(TCConstants.IMCMD_GIFT), new Gson().toJson(jsonData), new TRTCVoiceRoomCallback.ActionCallback() {
            @Override
     ...
     * 处理礼物弹幕消息
     */
    private void handleGiftMsg(TRTCVoiceRoomDef.UserInfo userInfo, String data) {
        if (mGiftInfoDataHandler != null) {
            GiftSendJson jsonData = new Gson().fromJson(data, GiftSendJson.class);
    ...
    public void onRecvRoomTextMsg(String message, TRTCVoiceRoomDef.UserInfo userInfo) {
        Log.d(TAG, "所有在该房间内的主播和听众均可以收到 onRecvRoomTextMsg 回调");
    ...
    public void onRecvRoomCustomMsg(String cmd, String message, TRTCVoiceRoomDef.UserInfo userInfo) {
        ...
        Log.d(TAG, "收到来自" + userInfo.userName + "礼物消息:" + message);
        Log.d(TAG, "userId: " + userInfo.userId);
        Log.d(TAG, "userName: " + userInfo.userName);
        Log.d(TAG, "userAvatar: " + userInfo.userAvatar);
    ...
    public void onInviteeAccepted(String id, String invitee) {
        Log.d(TAG, "听众端收到 onInviteeAccepted 的事件通知，调用 enterSeat 进行上麦 ");
    ...
    private View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            intent.putExtra(Constants.USERID, mMainSeatUserId);
    ...
    public static void show(Context context) {
        GiftAdapter giftAdapter = new DefaultGiftAdapterImp(); //初始化查询对像
        GiftInfoDataHandler mGiftInfoDataHandler = new GiftInfoDataHandler();
        mGiftInfoDataHandler.setGiftAdapter(giftAdapter);
        IGiftPanelView giftPanelView = new GiftPanelViewImp(context);
        giftPanelView.init(mGiftInfoDataHandler);
    ...
    private GiftPanelDelegate giftPanelDelegate = new GiftPanelDelegate() {
        ...
        public void onChargeClick() {
            Intent intent = new Intent();
            intent.setAction("com.paixide.activity.Withdrawal.Detailedlist");
            startActivity(intent);
        }
    };
