...
public class FriendProfileLayout extends LinearLayout implements View.OnClickListener {
    ...
    public void initData(Object data) {
        if (data instanceof ChatInfo) {
            ...
            mId = mChatInfo.getId();
            Log.d(TAG, "initData: 1111" + mChatInfo.toString());
            chattype =mChatInfo.getType();
            ...
            Log.d(TAG, "initData:2221 " + mContactInfo.toString());
            ...
            Log.d(TAG, "initData:3333 " + mFriendApplication.toString());
            ...
            Log.d(TAG, "initData:4444 " + mFriendApplication.toString());
    ...
    private void refuse() {
        V2TIMManager.getFriendshipManager().refuseFriendApplication(mFriendApplication, new V2TIMValueCallback<V2TIMFriendOperationResult>() {
            ...
            public void onError(int code, String desc) {
                //TUIKitLog.e(TAG, "accept err code = " + code + ", desc = " + desc);
                //ToastUtil.toastShortMessage("Error code = " + code + ", desc = " + desc);
                ToastUtil.toastShortMessage("操作失败请稍后再试");
            }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btnChat) {
            if (chattype == V2TIMConversation.V2TIM_GROUP) {
                ToastUtil.toastLongMessage(getContext().getString(R.string.tv_msg_tt20));
            } else {
                chat();
            }
