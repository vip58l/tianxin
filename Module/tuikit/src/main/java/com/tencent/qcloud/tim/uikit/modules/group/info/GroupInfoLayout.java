 ...
 * 群聊详情面板
 */
public class GroupInfoLayout extends LinearLayout implements IGroupMemberLayout, View.OnClickListener, GroupInfoAdapter.muteGroupMember {
    ...
    public LineControllerView forbidden_words;
    public LineControllerView deleteConversation_bar;
    ...
    private void init() {
        ...
        mMemberAdapter.setMuteGroupMember(this::muteGroupMember);
        ...
        //全体禁言
        forbidden_words = findViewById(R.id.forbidden_words);
        forbidden_words.setCheckListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    mPresenter.setGroupInfo(isChecked, "管理员设置全体禁言");
                } else {
                    mPresenter.setGroupInfo(isChecked, "管理员解除全体禁言");
        ...
        //删除聊天记录
        deleteConversation_bar = findViewById(R.id.self_deleteConversation_bar);
        deleteConversation_bar.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.group_member_bar) {
        ...
        } else if (v.getId() == R.id.self_deleteConversation_bar) {
            //删除聊天记录
            mPresenter.getmProvider().deleteConversation(new V2TIMCallback() {
                @Override
                public void onError(int i, String s) {
                    ToastUtil.toastLongMessage(getContext().getString(R.string.emptycontentfailed));
                }

                @Override
                public void onSuccess() {
                    ToastUtil.toastLongMessage(getContext().getString(R.string.Emptychatcontent));
            ...
            if (mGroupInfo.isOwner() && (!mGroupInfo.getGroupType().equals(TUIKitConstants.GroupType.TYPE_WORK) || !mGroupInfo.getGroupType().equals(TUIKitConstants.GroupType.TYPE_PRIVATE))) {
                new TUIKitDialog(getContext())
                        ...
                        .setPositiveButton(getContext().getString(R.string.sure), new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                //#群组已解散下架发送到服务器记录
                                GroupUpdate.desdeletegroup(mGroupInfo, null);
                            }
                        })
    ...
    private void setGroupInfo(GroupInfo info) {
        ...
        //重服务器获取禁言查询状态
        //mPresenter.selectgroupchat(this);

        //从腾讯API获取是否全群禁言
        forbidden_words.setChecked(info.getGroupInfo().isAllMuted());

        if (mGroupInfo.isOwner()) {
            //群主/管理员
            mGroupNotice.setVisibility(VISIBLE);
            mJoinTypeView.setVisibility(VISIBLE);
            forbidden_words.setVisibility(VISIBLE);
            if (mGroupInfo.getGroupType().equals(TUIKitConstants.GroupType.TYPE_WORK) || mGroupInfo.getGroupType().equals(TUIKitConstants.GroupType.TYPE_PRIVATE)) {
            ...
            forbidden_words.setVisibility(GONE);
            mDissolveBtn.setText(R.string.exit_group);
        }
    ...
    @Override
    public void muteGroupMember(String groupID, String userID, int var3) {
        Memberdocument(mGroupInfo, groupID, userID, var3);
    }

    /**
     * //https://cloud.tencent.com/document/product/269/1671#.E7.BE.A4.E7.BB.84.E9.94.99.E8.AF.AF.E7.A0.812
     *
     * @param mGroupInfo
     * @param groupID
     * @param userID
     * @param var3
     */
    public static void Memberdocument(GroupInfo mGroupInfo, String groupID, String userID, int var3) {
        if (mGroupInfo.isOwner()) {
            if (UserInfo.getInstance().getUserId().equals(userID)) {
                return;
            }
            Bundle bundle = new Bundle();
            bundle.putString(TUIKitConstants.Selection.TITLE, TUIKit.getAppContext().getResources().getString(R.string.group_no_chat));
            bundle.putString(TUIKitConstants.Selection.INIT_CONTENT, String.valueOf(var3));
            bundle.putInt(TUIKitConstants.Selection.LIMIT, 20);
            SelectionActivity.startTextSelection(TUIKit.getAppContext(), bundle, new SelectionActivity.OnResultReturnListener() {
                ...
                public void onReturn(Object res) {
                    ...
                    GroupInfoAdapter.muteGroupMembers(groupID, userID, TextUtils.isEmpty(s) ? var3 : i);
                }
