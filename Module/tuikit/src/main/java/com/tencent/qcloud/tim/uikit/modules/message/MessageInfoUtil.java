...
public class MessageInfoUtil {
     ...
     * @return
     */
    public static MessageInfo LocationMessage(String address, double longitude, double latitude) {
        MessageInfo info = new MessageInfo();
        V2TIMMessage v2TIMMessage = V2TIMManager.getMessageManager().createLocationMessage(address, longitude, latitude);
        info.setSelf(true);
        info.setTimMessage(v2TIMMessage);
        info.setMsgTime(System.currentTimeMillis() / 1000);
        info.setMsgType(MessageInfo.MSG_TYPE_LOCATION);
        info.setFromUser(V2TIMManager.getInstance().getLoginUser());
        info.setExtra(TUIKit.getAppContext().getString(R.string.tv_gps));
        return info;
    }
     ...
     * @return
     */
    public static MessageInfo snebuildCustomMessage(String date) {
        V2TIMMessage v2TIMMessage = V2TIMManager.getMessageManager().createTextMessage(date);
        V2TIMCustomElem customElem = new V2TIMCustomElem();
        customElem.setData(date.getBytes());
     ...
     * @return
     */
    public static void sendC2CCustomMessage(String cmd, String userid) {
        V2TIMManager.getInstance().sendC2CCustomMessage(cmd.getBytes(), userid,null);
     ...
     * @return
     */
    public static MessageInfo Call_VIDEO_Audio(int TYPE, List<UserModel> contactList) {
        MessageInfo info = new MessageInfo();
        info.setTYPE(TYPE);
        info.setContactList(contactList);
        return info;
    }
}
