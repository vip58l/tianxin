...
public class TUIKitImpl {
    ...
    private static TRTCVoiceRoom trtcVoiceRoom;
    private static final List<IMEventListener> sIMEventListeners = new ArrayList<>();
    private static Gson sGson;

    public static void init(Context context,TUIKitConfigs configs){
        sAppContext = context;
        sConfigs = configs;
    ...
    public static void login(final String userid, final String usersig, final IUIKitCallBack callback) {
        ...
        //TUIKitConfigs.getConfigs().getGeneralConfig().setSDKAppId(1400468644);
        ...
        V2TIMManager.getInstance().login(userid, usersig, new V2TIMCallback() {
            ...
            public void onSuccess() {
                ...
                //登录直播
                loginTUIKitLive(TUIKitConfigs.getConfigs().getGeneralConfig().getSDKAppId(), userid, usersig);
                //登录语音
                loginTRTCVoiceRoom(TUIKitConfigs.getConfigs().getGeneralConfig().getSDKAppId(), userid, usersig);
    ...
    private static void initIM(final Context context, int sdkAppID) {
        ...
        //添加好友成功后有相关回调，可以自行实现通知  设置关系链监听器
        ...
        V2TIMManager.getMessageManager().addAdvancedMsgListener(new V2TIMAdvancedMsgListener() {
            ...
            public void onRecvNewMessage(V2TIMMessage msg) {
                ...
                for (IMEventListener l : sIMEventListeners) {
                    l.onNewMessage(msg);
                    Log.d(TAG, "onRecvNewMessage: " + msg.getUserID());
                }
            }
        ...
        //接收文本和信令消息（简化接口）V2TIM消息管理器addSignalingListener
    ...
    public static void addIMEventListener(IMEventListener listener) {
        if (listener != null && sIMEventListeners.size() == 0) {
     ...
     * @param context
     */
    private static void TRTCVoiceRoom(Context context) {
        trtcVoiceRoom = TRTCVoiceRoom.sharedInstance(context);
    }
     ...
     * @param userSig
     */
    private static void loginTRTCVoiceRoom(int sdkAppid, String userId, String userSig) {
        trtcVoiceRoom.login(sdkAppid, userId, userSig, null);
    }

    /**
     * 退出语音聊天室
     */
    private static void trtcVoiceRoomlogout() {
        trtcVoiceRoom.logout(null);
    }
